...
function applyTheme(){var mode=state.theme||'auto',dark=mode==='dark'||(mode==='auto'&&window.matchMedia&&matchMedia('(prefers-color-scheme: dark)').matches);document.documentElement.setAttribute('data-theme',dark?'dark':'light');var meta=document.querySelector('meta[name="theme-color"]');if(meta)meta.setAttribute('content',dark?'#090f1d':'#101b46');if($('themeSelect'))$('themeSelect').value=mode}
function hash(s){var h=2166136261;for(var i=0;i<s.length;i++){h^=s.charCodeAt(i);h+=(h<<1)+(h<<4)+(h<<7)+(h<<8)+(h<<24)}return h>>>0}
function renderMotivation(){var dayNumber=Math.floor(new Date(today+'T00:00:00').getTime()/86400000),index=(dayNumber*7+hash('daily-motivation'))%MOTIVATION_MESSAGES.length,hour=new Date().getHours(),greeting=hour<11?'GOOD MORNING':hour<18?'KEEP GOING':'GOOD EVENING',parts=today.split('-');if($('dailyMotivation'))$('dailyMotivation').textContent=MOTIVATION_MESSAGES[index];if($('heroGreeting'))$('heroGreeting').textContent=greeting;if($('heroDate'))$('heroDate').textContent=parts[0]+'.'+parts[1]+'.'+parts[2]}
...
function renderStats(){var words=currentWords(),m=mission(),reviewed=cardsSeenCount(),remaining=Math.max(0,reviewWords().length-reviewed),ms=(m.quiz?1:0)+(m.cardsDone?1:0)+(m.writing?1:0);renderMotivation();$('score').textContent=(state.score||0)+'/'+(words.length||10);$('missionScore').textContent=ms+'/3';$('days').textContent=Object.keys(state.days||{}).length;$('streak').textContent=streak();$('exam').textContent=examDays();$('known').textContent=Object.keys(state.stats).filter(function(w){return state.stats[w].known}).length;$('reviewRemaining').textContent=remaining;$('startReviewBtn').classList.toggle('review-complete',remaining===0);$('startReviewBtn').querySelector('b').textContent=remaining===0?'今日の復習完了 ✓':'復習を始める →';$('homeDashboard').style.setProperty('--mission-progress',(ms/3*100)+'%');$('bar').style.width=(ms/3*100)+'%';$('missionQuiz').classList.toggle('done',!!m.quiz);$('missionCards').classList.toggle('done',!!m.cardsDone);$('missionWriting').classList.toggle('done',!!m.writing);$('cardsSeen').textContent=reviewed;$('status').textContent=m.complete?'Daily Mission Complete. 英検1級まであと '+examDays()+' 日。':'今日のMission: Quiz＋Flashcards＋Writing。英検1級まであと '+examDays()+' 日。';renderCalendar()}
