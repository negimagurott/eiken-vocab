(function(){
'use strict';
var VERSION='2.5',RELEASE='20260716-learning-calendar',KEY='eiken1_vocab_app_v20',OLD_KEYS=['eiken1_vocab_app_v13','eiken1_vocab_app_v12','eiken1_vocab_app_v11','eiken1_vocab_app_v5'],REVIEW_LIMIT=15,SRS_GAPS=[1,3,7,14,30];
...
function load(){try{var raw=localStorage.getItem(KEY);if(!raw){for(var i=0;i<OLD_KEYS.length;i++){raw=localStorage.getItem(OLD_KEYS[i]);if(raw)break}}if(raw)state=Object.assign(state,JSON.parse(raw))}catch(e){}if(!state.stats)state.stats={};if(!state.days)state.days={};if(!state.history)state.history={};if(!state.missions)state.missions={};if(!state.answers)state.answers={};if(!state.regenByDate)state.regenByDate={};if(!state.writingByDate)state.writingByDate={};if(!state.dailyReviewByDate)state.dailyReviewByDate={};if(!state.theme)state.theme='auto';Object.keys(state.missions).forEach(function(date){var m=state.missions[date]||{};if(m.quiz||m.writing||Object.keys(m.cards||{}).length)state.days[date]=true});WORDS.forEach(function(x){var s=state.stats[x.w];if(!s)s=state.stats[x.w]={seen:0,correct:0,wrong:0,known:false,due:'',level:0};if(typeof s.hardCount!=='number')s.hardCount=0;if(typeof s.quizAttempts!=='number')s.quizAttempts=s.seen||0;if(typeof s.quizCorrect!=='number')s.quizCorrect=s.correct||0;if(typeof s.reviewStep!=='number')s.reviewStep=Math.max(0,Math.min(4,s.level||0));if(!s.lastStudied)s.lastStudied=s.lastCardDate||'';if(!s.seen&&s.due==='2000-01-01')s.due=''});ensureMission();ensureDailyReview();save()}
...
function streak(){var n=0,d=state.days&&state.days[today]?today:addDays(today,-1);while(state.days&&state.days[d]){n++;d=addDays(d,-1)}return n}
...
function renderCalendar(){
  ...
  var currentMonth=today.slice(0,7),denominator=calendarMonth<currentMonth?daysInMonth:calendarMonth===currentMonth?Number(today.slice(8,10)):0,rate=denominator?Math.round(studied/denominator*100):0,currentStreak=streak();
  $('studyCalendar').innerHTML=html;$('calendarTitle').textContent=year+'年'+(month+1)+'月';$('monthStudyDays').textContent=studied+'日';$('monthStudyRate').textContent=denominator?rate+'%':'—';$('calendarNext').disabled=calendarMonth>=currentMonth;$('streakLarge').textContent=currentStreak;$('streakMessage').textContent=state.days&&state.days[today]?'今日も学習済み。この調子で続けましょう':currentStreak?'現在'+currentStreak+'日継続中。今日も記録をつなげましょう':'今日の学習から新しい記録を始めましょう';
}
...
function renderWords(){
  ...
  $('wordBox').innerHTML=arr.map(function(x){var st=state.stats[x.w]||{},label=reviewLabel(st.lastCardResult),attempts=st.quizAttempts||0,accuracy=attempts?Math.round((st.quizCorrect||0)/attempts*100)+'%':'—';return '<div class="word"><b>'+esc(x.w)+'</b><span class="pos-badge">'+esc(posLabel(x))+'</span><button class="speak" data-speak="'+esc(x.w)+'" type="button">🔊</button><p>'+esc(x.m)+'</p><p class="small"><b>例文：</b>'+esc(x.s.replace('____',x.w))+'</p>'+(x.collocations&&x.collocations.length?'<p class="small"><b>コロケーション：</b>'+esc(listText(x.collocations))+'</p>':'')+'<span class="pill">正答率 '+accuracy+'</span><span class="pill">誤答 '+(st.wrong||0)+'</span><span class="pill">まだ '+(st.hardCount||0)+'</span><span class="pill">次回 '+(st.due||'未設定')+'</span>'+(label?'<span class="pill '+(st.lastCardResult==='good'?'review-good':'review-hard')+'">'+label+'</span>':'')+'</div>'}).join('');
...
function switchTab(name){document.querySelectorAll('.tab').forEach(function(t){t.classList.toggle('active',t.dataset.tab===name)});document.querySelectorAll('.tabPanel').forEach(function(p){p.classList.toggle('hidden',p.id!==name)});if(name==='cards')syncCardHeight();if(name==='settings')exportData()}
