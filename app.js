(function(){
'use strict';
var VERSION='2.5.4',RELEASE='20260716-study-recovery-button',KEY='eiken1_vocab_app_v20',OLD_KEYS=['eiken1_vocab_app_v13','eiken1_vocab_app_v12','eiken1_vocab_app_v11','eiken1_vocab_app_v5'],REVIEW_LIMIT=15,SRS_GAPS=[1,3,7,14,30];
...
function load(){try{var raw=localStorage.getItem(KEY);if(!raw){for(var i=0;i<OLD_KEYS.length;i++){raw=localStorage.getItem(OLD_KEYS[i]);if(raw)break}}if(raw)state=Object.assign(state,JSON.parse(raw))}catch(e){}if(!state.stats)state.stats={};if(!state.days)state.days={};if(!state.history)state.history={};if(!state.missions)state.missions={};if(!state.answers)state.answers={};if(!state.regenByDate)state.regenByDate={};if(!state.writingByDate)state.writingByDate={};if(!state.dailyReviewByDate)state.dailyReviewByDate={};if(!state.theme)state.theme='auto';Object.keys(state.missions).forEach(function(date){var m=state.missions[date]||{};if(m.quiz||m.writing||Object.keys(m.cards||{}).length)state.days[date]=true});applyStudyRecovery();WORDS.forEach(function(x){var s=state.stats[x.w];if(!s)s=state.stats[x.w]={seen:0,correct:0,wrong:0,known:false,due:'',level:0};if(typeof s.hardCount!=='number')s.hardCount=0;if(typeof s.quizAttempts!=='number')s.quizAttempts=s.seen||0;if(typeof s.quizCorrect!=='number')s.quizCorrect=s.correct||0;if(typeof s.reviewStep!=='number')s.reviewStep=Math.max(0,Math.min(4,s.level||0));if(!s.lastStudied)s.lastStudied=s.lastCardDate||'';if(!s.seen&&s.due==='2000-01-01')s.due=''});ensureMission();ensureDailyReview();save()}
...
function restoreStudyDays(){state.days={};for(var i=0;i<7;i++)state.days[addDays(today,-i)]=true;state.studyRecovery='7-days-'+today;save();renderStats();toast('Study DaysとStreakを7日に復旧しました')}
function applyStudyRecovery(){try{var url=new URL(location.href);if(url.searchParams.get('restore')!=='7')return;state.days={};for(var i=0;i<7;i++)state.days[addDays(today,-i)]=true;state.studyRecovery='7-days-'+today;url.searchParams.delete('restore');history.replaceState(null,'',url.pathname+(url.searchParams.toString()?'?'+url.searchParams.toString():'')+url.hash)}catch(e){}}
